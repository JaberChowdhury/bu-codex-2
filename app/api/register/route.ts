...
import {
  ...
  memberSchema,
  teamNameSchema,
} from "@/components/register/schema"
...
export async function POST(req: Request) {
  try {
    ...
    if (typeof teamName !== "string" || typeof teamCode !== "string") {
      return NextResponse.json(
        { error: "Invalid teamName or teamCode" },
        { status: 400 }
      )
    }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (typeof memberStr !== "string") {
        return NextResponse.json(
          { error: `Missing data for member ${i + 1}` },
          { status: 400 }
        )
      }
      ...
      if (!memberResult.success) {
        return NextResponse.json(
          { error: `Validation failed for member ${i + 1}` },
          { status: 400 }
        )
      }
      ...
      if (!(photoFile instanceof File)) {
        return NextResponse.json(
          { error: `Missing or invalid photo for member ${i + 1}` },
          { status: 400 }
        )
      }
      ...
      if (uploadError) {
        ...
        return NextResponse.json(
          {
            error: `Failed to upload photo for member ${i + 1}: ${uploadError.message}`,
          },
          { status: 500 }
        )
      }
